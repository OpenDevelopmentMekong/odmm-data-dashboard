...
function ElectionPartyChart(config) {
  ...
  this.processAfterData = function (data) {
    $.map(self.charts, function (value, index) {
      if (data.result.records.length > 0) {
        ...
        var sorted = self.sortResults(
          data.result.records,
          self.resource.sort_by
        );
  ...
  this.addDataSourceLink = function (value) {
    if (self.resource.dataset_id !== undefined) {
      ...
      jQuery("#" + value.container_id).append(
        '<div class="resource_link">Data Source : <a href="' +
          dataset_url +
          '" target="_blank">' +
          self.resource.resource_title +
          "</a></div>"
      );
    }
  };
  ...
  this.prepareChart = function (data, chart_conf) {
    ...
    if (columns.length > 1) {
      ...
      googleChart.draw(
        chart_conf.chart_type,
        document.getElementById(chart_conf.container_id),
        chart_conf.ChartData,
        chart_conf.chart_options
      );
    }
  };
  ...
  this.getPartyColor = function (partyname) {
    ...
    return partyname == "National League for Democracy"
      ? "#F44336"
      : partyname == "Union Solidarity And Development Party"
      ? "green"
      : partyname == "Union Solidarity and Development Party"
      ? "green"
      : partyname == "Shan Nationalities Democratic Party"
      ? "#4CAF50"
      : partyname == "National Democratic Force"
      ? "#3F51B5"
      : partyname == "National Unity Party"
      ? "#FFEB3B"
      : partyname == "National Unity Party (NUP)"
      ? "#FFEB3B"
      : partyname == "Rakhine Nationals Development Party"
      ? "#9E9E9E"
      : partyname == "Arakan National Party"
      ? "#0288D1"
...
var PopulationPyramid = {
  ...
  getData: function (pcode) {
    return $.ajax({
      ...
      data: {
        ...
        filters:
          '{"pop_type" : "Total_population", "State_Region Pcode" : "' +
          pcode +
          '"}',
      },
      dataType: "json",
  ...
  processAfterData: function (data) {
    ...
    googleChart.draw(
      p_config.chart_type,
      document.getElementById(p_config.container_id),
      PopulationPyramidData,
      p_config.chart_options
    );

    var resource_link =
      data_source_url + "?id=7bc0cabc-3c01-44fe-ba30-943a360c56fb";

    var resource_title = "2014 Myanmar Population and Housing Census";

    jQuery("#" + p_config.container_id).append(
      '<div class="resource_link">Data Source : <a href="' +
        resource_link +
        '" target="_blank">' +
        resource_title +
  ...
  processData: function (data) {
    ...
    $.map(charts.st_population.fields, function (value, index) {
      PopulationPyramidData.addRow([
        value,
        -parseInt(MaleData[index]),
        parseInt(FemaleData[index]),
      ]);
...
var MinisterList = {
  init: function (pcode) {
    $.ajax({
      ...
      var list = jQuery(
        "<ul style='display:flex;flex-direction:row;justify-content:space-around;flex-flow:wrap;'>"
      );
      ...
      jQuery.map(data.result.records, function (value, index) {
        ...
        var minister_info =
        '<div class="minister_name card-header-title" style="font-weight:bold;">' +
        minister_name +
        '<i class="minister_gender card-image fa fa-' + value.gender + ' fa-fw" aria-hidden="true"></i>' +
        '<br/>' +
         '<p class="minister_pos card-content">' +
          value.position +
          '</p>' +
       '<p class="ministry_val card-content">' +
        minsitry_name +
        '</p> </div>';
          // +
          //'<i class="minister_gender card-image fa fa-' + value.gender + ' fa-border fa-pull-left" aria-hidden="true">'
          //'<div class="minister_gender fa fa-female" >' +
          //value.gender +
          //"</i>";

        var minister_item = jQuery("<li>").addClass("card is-quarter").append(minister_info);
