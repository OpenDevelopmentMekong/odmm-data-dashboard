  ...
  this.init = function (pcode) {
    ...
    $.map(this.charts, function (value, index) {
      if (
        ...
        self.charts[index]["SourceTableData"] = self.makeDataTable(
          value.columns
        );
          ...
          if (value.chart_type == "treemap") {
            ...
            value.chart_options.generateTooltip = function (row, size, value) {
              if (treechartdata.getValue(row, 2) > 0) {
                return (
                  '<div class="treemap_tooltip">' +
                  "<h5>" +
                  treechartdata.getValue(row, 0) +
                  "</h5>" +
                  self.formatNumber(treechartdata.getValue(row, 2)) +
                  "<br>" +
                  "</div>"
                );
              } else {
      ...
      resource_container.append(
        jQuery("<a>")
          ...
          .on("click", function () {
            self.downloadCSV(
              value.ChartData,
              value.container_id + "_" + self.pcode
            );
          })
      );
    }
