...
function ODChart(config) {
  ...
  this.init = function () {
    ...
    if (
      value.chart_type !== "text" ||
      (value.prepare_data_from_array !== undefined &&
        value.prepare_data_from_array === true)
    ) {
    ...
    this.getData()
      .done(function (data) {
        ...
        if (data.result.records.length >= 1) {
          ...
          $("#chart_js_error")
            .html(
              dashboard.config_error_msg +
                "<br> <span class='error_msg'> " +
                dashboard.no_data_error +
                "</span>"
            )
            .show();
        ...
        $("#chart_js_error")
          .html(
            dashboard.config_error_msg +
              ...
              errorMsg +
              "</span>" //Errors are not same need to check all possible ways.
          )
          .show();
      });
  };
  ...
  this.processAfterData = function (data) {
    ...
    if (value.chart_type == "text") {
      if (
        self.data[value.field] === undefined ||
        self.data[value.field] === null ||
        (self.data[value.field] === 0 && value.ignore_zero === true)
      ) {
      ...
      if (c_data === false) {
        ...
        if (value.data_source_table === true) {
          ...
          googleChart.draw(
            "table",
            document.getElementById(value.container_id + "_table_wrapper"),
            value.ChartData
          );
        }

        jQuery("#" + value.container_id + "_table_wrapper").prepend(
          self.getDataSourceLinkTemplate(value)
        );

        //Add Title for table if it's in config
        if (
          value.chart_type == "table" &&
          value.chart_options.title !== undefined
        ) {
          jQuery("#" + value.container_id).prepend(
            "<h5>" + value.chart_options.title + "</h5>"
          );
        }
      }
    }
  ...
  this.getDataSourceButtonTemplate = function (value) {
    return jQuery("<div>")
      .addClass("data_source_bar")
      .append(
        jQuery("<a>")
          .addClass("data_source_btn")
          .attr("data-target", value.container_id + "_table")
          .html(
            ' <span class="data_source_btn_text">' +
              dashboard.show_data +
              "<span>"
          )
          .prepend(jQuery("<i>").addClass("fa fa-cog"))
      );
  };

  this.getTableContainerTemplate = function (value) {
    return jQuery("<div>")
      .addClass("source-table-wrapper hide")
  ...
  this.downloadCSV = function (ChartData, filename) {
    ...
    for (i = 0; i < dt_rows; i++) {
      ...
      for (var j = 0; j < dt_cols; j++) {
        ...
        raw_col.push(
          ChartData.getFormattedValue(i, j, "label").replace(/,/g, "")
        );
      }
      // Add row to CSV text
